...
namespace AlekGames.HoverCraftSystem.Systems.Main
{
    ...
    public class hoverCraft : MonoBehaviour
    {
        ...
        public AudioClip startEngineSFX;
        public AudioClip idleEngineSFX;
        public AudioClip accelerationPowerOn; 
        public AudioClip accelerationFullPower;
        ...
        private float timer;
        private float timeToActivateThirdAudioSource = 8.8f;

        private bool accelerationSoundStopped = false;
        ...
        private void Update()
        {
            ...
            if (accelerationInput > 0f && !accelerationSound.isPlaying && !accelerationSoundStopped)
            {
                if (!fadingOut && !fadingIn)
                {
                    if (idleAudioSource.isPlaying)
                    {
                        StartCoroutine(FadeOutSound(idleAudioSource, 1f));
                        fadingOut = true;
                    }
        
                    StartCoroutine(FadeInSound(accelerationSound, 1f));
                    fadingIn = true;
                }
            }
            else if (accelerationInput == 0f && accelerationSound.isPlaying && !accelerationSoundStopped)
            {
                if (!fadingOut && !fadingIn)
                {
                    StartCoroutine(FadeOutSound(accelerationSound, 1f));
                    fadingOut = true;
                    if (!idleAudioSource.isPlaying)
                    {
                        idleAudioSource.Play();
                        StartCoroutine(FadeInSound(idleAudioSource, 1f));
                        fadingIn = true;
                    }
                    if (thirdAudioSource.isPlaying)
                    {
                        StartCoroutine(FadeOutSound(thirdAudioSource, 1f));
                    }
                }
            }
        
            if (accelerationInput == 0f && (accelerationSound.isPlaying || thirdAudioSource.isPlaying))
            {
                if (!fadingOut && !fadingIn)
                {
            ...
            if (accelerationInput > 0f)
            {
                ...
                if (timer >= timeToActivateThirdAudioSource && !thirdAudioSource.isPlaying)
                {
                    if (!fadingOut && !fadingIn)
                    {
                        StartCoroutine(FadeOutSound(accelerationSound, 1f));
                        StartCoroutine(FadeInSound(thirdAudioSource, 1f));
                        fadingOut = true;
                        fadingIn = true;
                    }
                }
                
                if (timer >= 8f && !thirdAudioSource.isPlaying)
                {
        ...
        IEnumerator FadeOutSound(AudioSource audioSource, float fadeTime)
        {
            ...
            fadingOut = false;
            if (audioSource == accelerationSound)
            {
                accelerationSoundStopped = true;
            }
        }

        IEnumerator FadeInSound(AudioSource audioSource, float fadeTime)
        {
        ...
        {
            idleAudioSource.PlayOneShot(startEngineSFX);
            idleAudioSource.loop = false; 
            yield return new WaitForSeconds(0.4f);

            // Iniciar el fade in del segundo sonido
            float elapsedTime = 0f;
            float fadeTime = 2f; // Duración del fade in en segundos
            idleAudioSource.clip = idleEngineSFX;
            idleAudioSource.loop = true; 
            idleAudioSource.Play();
            while (elapsedTime < fadeTime)
            {
                elapsedTime += Time.deltaTime;
                idleAudioSource.volume = Mathf.Lerp(0f, 1f, elapsedTime / fadeTime);
                yield return null;
            }
        }
